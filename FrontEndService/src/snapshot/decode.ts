...
export function decodeSnapshot(buffer: ArrayBuffer, knownIds?: ReadonlySet<number>): SnapshotFrame {
  ...
  if (HEADER_BYTES + payloadBytes !== buffer.byteLength)
  ...
  if (expectedCrc !== 0 && crc32c(payload) !== expectedCrc)
    throw new Error('Snapshot CRC32C mismatch')
  ...
  for (let i = 0; i < sectionCount; i += 1) {
    ...
    const section: Section = {
      code: view.getUint16(base, true),
      type: view.getUint8(base + 2),
      components: view.getUint8(base + 3),
      offset: view.getUint32(base + 4, true),
      bytes: view.getUint32(base + 8, true),
      count: view.getUint32(base + 12, true),
    }
    const width = TYPE_BYTES[section.type]
    if (!width || section.count !== count || section.bytes !== count * section.components * width)
      throw new Error(`Invalid section 0x${section.code.toString(16)}`)
    if (
      section.offset < HEADER_BYTES + directoryBytes ||
      section.offset + section.bytes > buffer.byteLength
    )
      throw new Error('Section offset is outside frame')
  ...
  if (knownIds)
    for (let index = 0; index < ids.length; index += 1) {
      if (!knownIds.has(ids[index]!))
        throw new Error('Unknown aircraft integer ID; full-state resync required')
    }
  return {
    epochId: bytesToUuid(new Uint8Array(buffer, 16, 16)),
    sequence: view.getBigUint64(32, true),
    tickIndex: view.getBigUint64(40, true),
    tS: view.getFloat64(48, true),
    aircraftCount: count,
    aircraftIds: ids,
