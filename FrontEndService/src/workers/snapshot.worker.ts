...
self.onmessage = (
  event: MessageEvent<{ type: 'static'; ids: number[] } | { type: 'frame'; buffer: ArrayBuffer }>,
) => {
  if (event.data.type === 'static') {
    knownIds = new Set(event.data.ids)
    return
  }
  try {
    ...
    self.postMessage({
      type: 'error',
      message: cause instanceof Error ? cause.message : 'Snapshot decode failed',
    })
  }
}
