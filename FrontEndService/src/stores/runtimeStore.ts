...
const initial: RuntimeReadModel = {
  epoch_id: '',
  generation: 0,
  tick_index: 0,
  t_s: 0,
...
export const useRuntimeStore = defineStore('runtime', () => {
  const model = ref<RuntimeReadModel>({ ...initial }),
    pending = ref(false),
    error = ref('')
  const state = computed(() => model.value.session_state)
  function apply(next: RuntimeReadModel) {
    model.value = structuredClone(next)
    useSessionStore().syncRuntime(next.session_state, next.epoch_id)
  }
  async function control(
    ...
    error.value = ''
    try {
      return await gatewayTransport.control(session.scenarioId, session.epochId, operation, args)
    } catch (cause) {
      error.value = cause instanceof Error ? cause.message : 'Control request failed'
      return undefined
    } finally {
      pending.value = false
    }
