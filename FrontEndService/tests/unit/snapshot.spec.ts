...
describe('ViewerSnapshot v1 decoder', () => {
  it('returns zero-object TypedArray views for the required hot sections', () => {
    const frame = decodeSnapshot(
      createMockSnapshot({ epochId, sequence: 4, tick: 3, tS: 0.3, count: 6 }),
      new Set([1, 2, 3, 4, 5, 6]),
    )
  ...
  it('rejects CRC corruption and unknown static IDs', () => {
    ...
    const bytes = new Uint8Array(corrupted),
      last = corrupted.byteLength - 1
    bytes[last] = (bytes[last] ?? 0) ^ 0xff
    expect(() => decodeSnapshot(corrupted)).toThrow('CRC32C')
    expect(() =>
      decodeSnapshot(
        createMockSnapshot({ epochId, sequence: 1, tick: 1, tS: 0.1, count: 2 }),
        new Set([1]),
      ),
    ).toThrow('Unknown aircraft')
  })
})
