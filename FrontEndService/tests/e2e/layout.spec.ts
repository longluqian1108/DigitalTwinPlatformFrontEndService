...
async function metrics(page: Page): Promise<LayoutMetrics> {
  return page.evaluate(() => {
    ...
    const shell = rect('.shell'), body = rect('.body'), left = rect('.left'), map = rect('.map'), right = rect('.right'), dock = rect('.shell > footer')
    const stream = document.querySelector<HTMLElement>('.stream')
    if (!stream) throw new Error('Missing event stream')
    return {
      viewport: { width: window.innerWidth, height: window.innerHeight }, shell: { width: shell.width, height: shell.height },
      body: { top: body.top, bottom: body.bottom, height: body.height }, left: { width: left.width, height: left.height },
      map: { width: map.width, height: map.height }, right: { width: right.width, height: right.height },
      dock: { top: dock.top, height: dock.height }, stream: { clientHeight: stream.clientHeight, scrollHeight: stream.scrollHeight }
    }
  })
}

async function buildMockScenario(page: Page) {
  const root = process.cwd(), inputs = page.locator('input[type=file]')
...
test('keeps the command dock fixed while 80 events scroll inside their panel', async ({ page }) => {
  ...
  await page.evaluate(() => {
    const stream = document.querySelector('.stream'), original = stream?.querySelector('.event')
    const event = original ?? Object.assign(document.createElement('button'), { className: 'event', textContent: 'Synthetic realtime event' })
...
test('persists dragged ratios and reapplies them after reload and resize', async ({ page }) => {
  ...
  const saved = await page.evaluate(() => JSON.parse(localStorage.getItem('lbs.layout.v2') ?? '{}') as { version?: number; leftRatio?: number; rightRatio?: number; dockRatio?: number })
  ...
  const reloadedShare = reloaded.left.width / (reloaded.left.width + reloaded.map.width + reloaded.right.width)
  const resizedShare = resized.left.width / (resized.left.width + resized.map.width + resized.right.width)
